...
public class Register {
	...
	public void user_enters_details_into_below_fields(DataTable dataTable) {
		 ...
		 registerpage.enterInputEmailField(commonUtils.getEmailWithTimeStamp());
		 registerpage.entertelephoneField(dataMap.get("telephone"));
		 registerpage.enterInputPasswordField(dataMap.get("password"));
		 registerpage.enterInputPasswordConfirmField(dataMap.get("password"));
	...
	public void User_enters_details_into_below_fields_with_duplicate_email(DataTable dataTable) {
		 ...
		 registerpage.enterFirstName(dataMap.get("firstName"));
		 registerpage.enterLastName(dataMap.get("lastName"));
		 registerpage.enterInputEmailField(dataMap.get("email"));
		 registerpage.entertelephoneField(dataMap.get("telephone"));
		 registerpage.enterInputPasswordField(dataMap.get("password"));
		 registerpage.enterInputPasswordConfirmField(dataMap.get("password"));
		 //driver.findElement(By.id("input-firstname")).sendKeys(dataMap.get("firstName"));
	...
	public void user_select_yes_for_news_letter() {
		...
		registerpage.SeletcNewsLetterOption();
		//driver.findElement(By.xpath("//input[@name='newsletter'][@value='1']")).click();
	    
	}

	@Then("User account should get a proper warning about duplicate email address")
	public void user_account_should_get_a_proper_warning_about_duplicate_email_address() {
	    
		
		Assert.assertTrue(registerpage.RetrieveWarningMessageText().contains("Warning: E-Mail Address is already registered!"));
	}
	...
	public void user_should_get_proper_warning_message() {
	    ...
	    Assert.assertTrue(registerpage.RetrieveWarningMessageText().contains("Warning: You must agree to the Privacy Policy!"));
	    Assert.assertEquals("First Name must be between 1 and 32 characters!", registerpage.FirstNameWarning());
	    Assert.assertEquals("Last Name must be between 1 and 32 characters!", registerpage.LastNameWarning());
	    Assert.assertEquals("E-Mail Address does not appear to be valid!", registerpage.EmailWaning());
	    Assert.assertEquals("Telephone must be between 3 and 32 characters!", registerpage.TelephoneWarning());
	    Assert.assertEquals("Password must be between 4 and 20 characters!", registerpage.PasswordWarning());
